...
<template>
  <RouterLink :to="'/brawler/' + encodeURIComponent(brawler.id)">
    <li
      class="transition grid grid-cols-brawler-list grid-area-template-brawler-list md:grid-cols-brawler-list-md md:grid-area-template-brawler-list-md rounded-md overflow-hidden bg-zinc-800 hover:bg-zinc-700 p-2 md:p-3 mb-1 items-center"
    >
      <!--     rounded-md overflow-hidden bg-zinc-800 hover:bg-zinc-700 p-2 md:p-3 mb-1 -->

      <div class="w-16 grid-area-icon">
        <img :src="brawlerData?.imageUrl" :alt="_.capitalize(brawler.name) + ' icon'" />
      </div>
      <div class="pl-2 grid-area-name">
        {{ _.capitalize(props.brawler.name) }}
      </div>
      <div class="grid-area-rank">
        <!-- <span class="font-emoji">🥇</span> -->
        <BadgeRank :rank="props.brawler.rank" />
        <!-- {{ props.brawler.rank }} -->
      </div>
      <div class="grid-area-power">
        <BadgePower :level="props.brawler.power" />
      </div>
      <div class="inline-flex items-center gap-1 grid-area-trophies">
        <span class="font-emoji">🏆</span>
        <!-- <svg
